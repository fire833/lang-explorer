...
def data_viz(args):
	...
	plain = plain.removeprefix("embeddings_128_")
	...
	plt.tight_layout()
	plt.savefig(f"{args.output}/tsne2d{plain}colors.svg", dpi=300)
	plt.close()
	...
	plt.savefig(f"{args.output}/tsne2d{plain}ispartial.svg", dpi=300)
	plt.close()
